...
document.addEventListener('DOMContentLoaded', async function () {
    firebase.auth().onAuthStateChanged(async function (user) {
        ...
        document.getElementById('previousBtn').addEventListener('click', previousFlashcard);
    });
...
function previousFlashcard() {
    currentIndex = (currentIndex - 1 + Object.keys(questions).length) % Object.keys(questions).length;
    renderFlashcards();
}
...
async function fetchFlashcards() {
    try {
        ...
        const response = await fetch('https://us-central1-speechbuddy-30390.cloudfunctions.net/processURL', {
            method: 'POST',
            headers: {
...
function generatePrompt(goal, gradeLevel, pillar, additionalParams) {
    switch (pillar) {
        ...
        case 'Phonology':
            ...
            } else if (goal === 'Assimilation' && additionalParams.assimilation !== 'Consonant Deletion') {
...
function uploadAudio(audioBlob) {
    ...
    const audioFileName = uid + "_" + session.toString() + "_" + (currentIndex).toString() + '.wav';
