...
document.addEventListener('DOMContentLoaded', function () {
        ...
        loginButton.addEventListener('click', () => {
            ...
            firebase.auth().signInWithEmailAndPassword(email, password)
                .then(async userCredential => {
                    ...
                    saveUserToCookie({ email: email, subscribed: subscribed, uid: user.uid, customerId: firebaseDoc.customerId });
                    navigateTo('home.html');
                    hideLoader();
    ...
    if (resetPasswordButton) {
        resetPasswordButton.addEventListener('click', () => {
            ...
            if (!email) {
                hideLoader();
                document.getElementById('reset-error-message').textContent = 'Please enter your email first.';
                return;
            }
            ...
            firebase.auth().sendPasswordResetEmail(email)
                .then(() => {
                    ...
                    document.getElementById('reset-error-message').textContent = 'Password reset email sent';
                })
                .catch(error => {
                    hideLoader();
...
async function isSubscribed(customerID) {
    ...
    try {
        const response = await fetch("https://us-central1-speechbuddy-30390.cloudfunctions.net/getSubscriptions", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
